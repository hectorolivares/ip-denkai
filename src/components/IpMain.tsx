...
export function IpMain({ ipData, isp, fetchIpData }: IpMainProps) {
  ...
  return (
    <div className="flex flex-col items-center w-full px-4 pt-8 pb-4 relative">
      ...
      {copiedText && (
        <div className="absolute top-0 bg-[var(--accent)] text-white text-[11px] font-bold px-3 py-1 rounded-full animate-bounce shadow-lg z-50">
      ...
      <h1 
        className="text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-black tracking-tighter mb-4 text-[var(--text-h)] drop-shadow-sm font-mono cursor-pointer hover:scale-105 transition-transform select-none"
      ...
      {ipData.ipv6 && ipData.ipv6 !== ipData.ip && (
        <div className="mb-6 flex flex-col items-center">
          ...
          <span 
            className="text-[16px] md:text-[18px] font-mono text-[var(--accent)] bg-[var(--accent-bg)] px-4 py-1 rounded-full border border-[var(--accent-border)] shadow-sm cursor-pointer hover:scale-105 transition-transform select-none"
      ...
      <p className="text-[14px] opacity-80 max-w-2xl text-center mb-10 leading-relaxed">
        Conectado desde{" "}
        <strong className="text-[var(--text-h)] font-semibold">{ipData.city}, {ipData.country}</strong> a través de la red de{" "}
